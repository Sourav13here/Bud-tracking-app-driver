...
const MapScreen = () => {
  ...
  const [mapHeight] = useState(new Animated.Value(screenHeight * 0.6));
  ...
  // OPTIMIZED: Throttled scroll handler with state tracking
  const handleScrollEnd = useCallback((event) => {
    ...
    if (offsetY > 20 && isMapExpanded) {
      ...
      Animated.timing(mapHeight, {
        toValue: screenHeight * 0.6, // Expand to 60%
        duration: 250, // Faster animation
        useNativeDriver: false,
      }).start(() => {
        animationInProgress.current = false;
  ...
  // OPTIMIZED: Use onMomentumScrollEnd instead of onScroll
  ...
  // Stoppage item component - OPTIMIZED: Added React.memo equivalent
